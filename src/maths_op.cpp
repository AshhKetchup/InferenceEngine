...
void rmsnorm(float *out, const float *x, const float *g, size_t n, float eps) {
  // Pass 1: sum of squares (double accumulator for stability).
  double sumsq = 0.0;
  for (size_t i = 0; i < n; ++i) {
    sumsq += static_cast<double>(x[i]) * x[i];
  }

  // Compute inv RMS once; multiply (don't divide) in the inner loop.
  float mean_sq = static_cast<float>(sumsq / static_cast<double>(n));
  float inv_rms = 1.0f / std::sqrt(mean_sq + eps);

  // Pass 2: scale and apply per-channel weight. Safe in-place because each
  // out[i] depends only on x[i] (not on x[j != i]).
  for (size_t i = 0; i < n; ++i) {
...
static void test_rmsnorm() {
  ...
  // Example 2: weights scale per channel.
  float x2[] = {1, 2, 2};
  float g2[] = {2, 0, 1};
  float out2[3];
  ...
  // Example 4: single non-zero, direction preserved.
  float x4[] = {3, 0, 0};
  float g4[] = {1, 1, 1};
  float out4[3];
